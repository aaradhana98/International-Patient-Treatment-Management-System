 ...
 * 		 @AutoConfigureMockMvc can be applied to a test class to enable 
 * 		 and configure auto-configuration of MockMvc
 */
...
class InsuranceClaimControllerTest {
	...
	/**
	 * @MockBean allow to mock a class or an interface and to record and verify behaviors on it
	 */
	@MockBean
	 ...
	 */
	@Test
	public void test_GetAllInsurerDetail() throws Exception {
	 ...
	 */
	@Test
	public void test_GetInsurerDetail() throws Exception {
		...
		when(authClient.verifyToken("Bearer Token")).thenReturn(new AuthResponse("admin1", "admin1", true));
		MvcResult result=mockMvc.perform(get("/iptms/getinsurerbypackagename/insurerPackageName").header("Authorization", "Bearer Token")).andReturn();
		String expected="{\"id\":1,\"insurerName\":\"insurerName\",\"insurerPackageName\":\"insurerPackageName\",\"insuranceAmountLimit\":200.0,\"disbursementDuration\":8}";
		String actual=result.getResponse().getContentAsString();
		assertEquals(expected, actual);
	}
	 ...
	 */
	@Test
	public void test_InitiateClaim() throws Exception {
		InitiateClaim initiateClaim = new InitiateClaim(1l, "patientName", "ailment", "treatmentPackageName", "insurerName");
		when(insuranceClaimServiceImpl.initiateClaim("Bearer Token", initiateClaim)).thenReturn((double) 15000);
		when(authClient.verifyToken("Bearer Token")).thenReturn(new AuthResponse("admin1", "admin1", true));
		MvcResult result=mockMvc.perform(post("/iptms/initiateclaim").content(asJsonString(initiateClaim)).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).header("Authorization", "Bearer Token")).andReturn();
		String expected="15000.0";
		String actual=result.getResponse().getContentAsString();
		assertEquals(expected, actual);
	}
